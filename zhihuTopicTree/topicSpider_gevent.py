...
def get_topics(spider,id,topic_f,topic_log_f):
    ...
    html_dir = os.path.join(CUR_DIR,DATA_STORAGE,NOW)
...
class ZhihuSpider(object):
    ...
    def crawl_by_layer(self):
        ...
        while True:
            ...
            now = NOW
            data_dir = os.path.join(current_dir,DATA_STORAGE,now)
            ...
            results = []
            jobs_list = [gevent.spawn(multi_run_wrapper,params) for params in tmp]

            jobs = []
            for a_job in jobs_list:
                if len(jobs)< REQUEST_PER_TIME:
                    jobs.append(a_job) 
                else:
                    gevent.joinall(jobs)
...
def test():
    ...
    id = '19776749'
    topic_filename = 'test1.txt'
    topic_log_f = file('test1.log','w')
    print get_topics(spider,id,topic_filename,topic_log_f)
